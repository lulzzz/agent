...
func IsDatasetReadOnly(dataset string) bool {
	out, err := exec.Command("/bin/bash", "-c", "/var/lib/snapd/hostfs/sbin/zfs get readonly -H "+dataset+" | awk '{print $3}' ").CombinedOutput()
...
func RemoveDataset(dataset string) {
	out, err := exec.Command("/bin/bash", "-c", "/var/lib/snapd/hostfs/sbin/zfs destroy -r "+dataset).CombinedOutput()
	log.Check(log.ErrorLevel, "Removing zfs dataset "+string(out), err)
}

func CreateDataset(dataset string) {
	out, err := exec.Command("/bin/bash", "-c", "/var/lib/snapd/hostfs/sbin/zfs create "+dataset).CombinedOutput()
	log.Check(log.ErrorLevel, "Creating zfs dataset "+string(out), err)
}

func ReceiveStream(delta string, destDataset string) {
	out, err := exec.Command("/bin/bash", "-c", "cat "+delta+" | /var/lib/snapd/hostfs/sbin/zfs receive "+destDataset).CombinedOutput()
