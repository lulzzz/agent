...
func IsDatasetReadOnly(dataset string) bool {
	out, err := exec.Command("/bin/bash", "-c", "`which zfs` get readonly -H "+dataset+" | awk '{print $3}' ").CombinedOutput()
...
func RemoveDataset(dataset string) {
	out, err := exec.Command("/bin/bash", "-c", "`which zfs` destroy -r "+dataset).CombinedOutput()
	log.Check(log.ErrorLevel, "Removing zfs dataset "+string(out), err)
}

func CreateDataset(dataset string) {
	out, err := exec.Command("/bin/bash", "-c", "`which zfs` create "+dataset).CombinedOutput()
	log.Check(log.ErrorLevel, "Creating zfs dataset "+string(out), err)
}

func ReceiveStream(delta string, destDataset string) {
	out, err := exec.Command("/bin/bash", "-c", "cat "+delta+" | `which zfs` receive "+destDataset).CombinedOutput()
